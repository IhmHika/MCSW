...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.commandName === 'ping') {
        return await interaction.reply({ content: `Latency: ${client.ws.ping}ms`, ephemeral: true });
    }
    ...
    if (interaction.commandName === 'info') {
        ...
        const infoEmbed = new EmbedBuilder()
            ...
            .setColor(mcConnection ? 0x2ecc71 : 0xe74c3c)
            .addFields(
    ...
    if (interaction.commandName === 'start') {
        ...
        wss.on('connection', (ws) => {
            ...
            ws.on('close', () => { 
                ...
                // 切断：濃い赤 (0x992d22)
                channel?.send({ embeds: [new EmbedBuilder().setTitle('Disconnected').setColor(0x992d22)] }); 
            });
        });
    }
    ...
    if (interaction.commandName === 'log') {
        ...
        if (action === 'view') {
            ...
            await interaction.reply({ embeds: [new EmbedBuilder().setDescription('Logs cleared.').setColor(0x95a5a6)] });
        }
    }
